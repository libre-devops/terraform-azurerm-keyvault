...
# Minimal call: one key vault with the secure defaults (RBAC, deny-by-default network ACL, 90 day
# soft-delete). purge_protection is disabled only so this disposable example vault can be torn down;
# leave it at the module default (true) for real vaults.
module "keyvault" {
  source = "../../"
